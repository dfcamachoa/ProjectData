"""Silver Stage E — object-grain change data capture (silver_spec §3.5), pure core.

The one Silver sub-stage with **no PoC predecessor**. Its whole job is to separate
*engineering change* from *re-export churn*: SmartPlant re-exports the entire
drawing XML for a single symbol move, and — worse — **re-mints the element UID
when an object is deleted and recreated** (a routine SPPID rework action). So the
UID cannot be the identity key; it is demoted to an audit field.

Identity is therefore a **hierarchical anchor-match** that survives delete+recreate
and reordering (§3.5):

    equipment  -> its tag
    segment    -> (drawing_number, composed seg business tag)
    component  -> the bucket (owning-segment anchor, component_class), with members
                  paired inside the bucket

and three cleanly separated hashes:

    anchor_hash         matching — the business/structural anchor. NO UID.
    content_hash_eng    Modify + recompute trigger — engineering attributes PLUS
                        one-hop neighbour sets keyed on each neighbour's ANCHOR
                        identity (never its UID), undirected and directed hashed
                        separately. This is what makes an object AND its
                        neighbourhood immune to a neighbour's instance churn.
    content_hash_audit  content_hash_eng + the UID + the quarantined oracle fields
                        (src_turnover / src_subsystem) — makes a delete+recreate and
                        a turnover reassignment VISIBLE to the audit trail while
                        staying INERT for engineering CDC (§5 firewall preserved).
...
`content_hash_eng`) produces **zero** deltas — the acceptance test of §3.5.

Spark-free and unit-tested. The Spark job (`cdc_job.py`) assembles the two
versions and the neighbour-anchor sets and calls `diff`.
"""
...
def content_hash_eng(o: dict, grain: str) -> str:
    """Modify + recompute trigger. Excludes the UID and the oracle (§5)."""
...
def _match_bucket(olds: List[dict], news: List[dict], grain: str, anchor: tuple) -> List[dict]:
    """Pair the N old against N new members inside one anchor bucket (§3.5):
...
def diff(old_objs: List[dict], new_objs: List[dict], grain: str) -> List[dict]:
    ...
    grain. Emits New / Modified / Deleted (not Unchanged) — the interval open/close
    events Gold consumes. Delete+recreate of an unchanged object yields nothing."""
...
def enrich_neighbors(segments: List[dict], components: List[dict],
    ...
    `flow_neighbor_anchors` on every object of ONE drawing-version, keyed on each
    neighbour's ANCHOR (never its UID) — the property that makes CDC immune to a
    neighbour's delete+recreate (silver_spec §3.5). Mutates the dicts in place.
    `connections` are that version's `silver_connections` rows."""
...
def summarize(deltas: List[dict]) -> Dict[str, int]:
    out = {"New": 0, "Modified": 0, "Deleted": 0}
    for d in deltas:
        out[d["change_type"]] = out.get(d["change_type"], 0) + 1
    out["total"] = len(deltas)
    return out
