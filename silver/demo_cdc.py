...
`write_narrative(dest)` writes four PostProc XMLs — two drawings at Rev C (issued
for HAZOP) and Rev D (re-issued for design) — into ``dest/rev1_C`` and
``dest/rev2_D``. Every element UID is re-minted between the revisions (as
...
``narrative_project_b/NARRATIVE.md``; the expected result is 15 real deltas and 0
false positives. These are synthetic, scrubbed fixtures — illustrative structure,
confirm names against real exports.
"""
...
def _e(v):
    """XML-escape an attribute value — & < > and the inch-mark \" — so the file is
    well-formed and parses with the plain parser (no lxml recovery needed)."""
    return escape(str(v), {'"': "&quot;"})
