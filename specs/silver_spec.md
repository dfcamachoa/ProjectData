...
| **Prefix integrity** — segment Tag begins with subline Tag begins with Pipeline-System Tag | MD §2.2a/§2.3, §4.2 | custom cross-column | flag; retain |
...
| **Unmatched OPC** | `_stitch_from` `opc_offset`; ALG §5 D5; MD §2.11 | connectivity expectation on the assembled graph | **not a failure** — open boundary, flag (system continues off-set) |
...
| 2 | **Connectivity checks run on the raw graph** — orphan/dangling expectations fire on `_raw_topology()` and fail every inline valve. | All connectivity expectations run on the **reconstructed** `und`/assembled graph; raw `<Connection>` is never a QC surface (§2, §3.4). |
| 3 | **Oracle leakage** — a Silver computation (CDC, hierarchy, a join) reads `seg_sys`/`seg_sub`, making the 97% circular. | Quarantine discipline (§5): carry-don't-compute, physical column isolation, and a hard-fail structural invariant that no compute input derives from the oracle. |
| 4 | **GX abort-on-fail kills honest-partial-result** — default gating drops whole drawings on a flaggable issue (`algorithm_spec.md` §11). | Quarantine-and-flag posture for all but two cases (unparseable input; structural-invariant breach) (§3.4). |
| 5 | **Provenance loss on derived edges** — `silver_connections` omits the `derived` flag, so the semantic layer asserts inferred topology as source truth. | `derived` is a mandatory column; a Derived edge without it is a structural-invariant failure (§3.4, §4); flag originates in `Connection.derived` (§2). |
| 6 | **Direction dropped in Silver** — only `und` persisted, so the three directional guards can never be authored above (`walk.py`). | Silver emits both `und` and directed `adj` (`flowsTo`) as `silver_connections` rows with `directed` (§3.2, §4). |
